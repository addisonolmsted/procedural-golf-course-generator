...
/// Meander wander: a per-channel sinusoidal steering signal. The planform
/// instrument (examples/planform.rs vs tools/macro_campaign/real_planform.py)
/// measured pure thalweg steering 2–4× straighter than the real corpus
/// (600 m-window sinuosity p50 1.01–1.03 generated vs 1.06–1.10 real) with
/// 43–46% of channel length in sustained parallel runs vs 3–11% real —
/// gradient descent on a smooth macro field is locally uniform, so paths
/// beeline. Real channels wander even on uniform slopes (bar-pool dynamics
/// the macro field can't see); this injects that wander as a desired-turn
/// signal the terrain score can still override where the topography is
/// strong. Decorrelated phases per channel also break up parallel banding.
#[derive(Clone, Copy, Debug)]
pub struct Wander {
    /// Heading oscillation amplitude, radians (±).
    pub theta: f64,
    /// Meander wavelength along the path, metres.
    pub lambda_m: f64,
    pub phase: f64,
}

/// Position-seeded wander parameters (doctrine: integer- and position-
/// seeded, never sequence-seeded — output invariant to processing order).
pub fn wander_from(p: Vec2, salt: u64) -> Wander {
    let mix = |mut z: u64| {
    ...
    let cell = |v: f64| (v / 8.0).floor() as i64 as u64;
    let h0 = mix(cell(p.x) ^ cell(p.y).rotate_left(32) ^ salt.rotate_left(17));
    let u = |h: u64| (h >> 11) as f64 / (1u64 << 53) as f64;
    let (h1, h2, h3) = (mix(h0), mix(mix(h0)), mix(mix(mix(h0))));
    Wander {
        theta: 0.40 + 0.35 * u(h1),
        lambda_m: 500.0 + 650.0 * u(h2),
        phase: std::f64::consts::TAU * u(h3),
    }
}

/// Desired signed turn for the step from arc `s` to `s + step`: the change
/// of the wander signal's desired heading over that step.
pub fn wander_turn(w: &Wander, s: f64, step_m: f64) -> f64 {
    let k = std::f64::consts::TAU / w.lambda_m;
    w.theta * (libm::sin(k * (s + step_m) + w.phase) - libm::sin(k * s + w.phase))
}

/// Absolute desired heading offset at arc `s`. Used where the base heading
/// is recomputed from scratch every step (the infill blend) — a per-step
/// TURN would be diluted by the momentum term there, while an offset
/// applied after the blend survives it.
pub fn wander_heading(w: &Wander, s: f64) -> f64 {
    let k = std::f64::consts::TAU / w.lambda_m;
    w.theta * libm::sin(k * s + w.phase)
}
...
pub fn grow(
    ...
    for &u in jitters.iter().take(steps) {
        // Desired wander turn for this step (0 without a wander signal).
        let th_des = wander.map_or(0.0, |w| wander_turn(&w, arc_s, step_m));
        ...
        for k in -5i32..=5 {
            ...
            // Parent-corridor repulsion. The planform instrument's relation
            // breakdown showed ~95% of all parallel-run length is a child
            // tracking its OWN parent at 40–200 m: steering pulls the child
            // back into the same macro low the parent occupies, the ±40 m
            // trim kills anything that re-touches, and what survives is
            // exactly the population that glides just outside the floor
            // (real corpus: 3–11% parallel; generated before this: 43–46%).
            // Past the junction-leaving window, candidates that stay inside
            // the parent's 250 m corridor pay proportionally — the child is
            // pushed up the flank, where real tributaries live.
            let parent_pen = match avoid_parent {
                Some(par) if arc_s > 150.0 => {
                    let (_, dpar) = nearest_side(par, q);
                    (1.0 - dpar / 250.0).max(0.0) * 20.0
                }
                _ => 0.0,
            };
            // Wander pull: same metres-equivalent form as turn_pen, centred
            // on the desired turn instead of straight-ahead. Weight 30 vs
            // turn_pen's 14 means the realized turn lands ~2/3 of the way
            // to the desired one on featureless ground, while a real valley
            // (metre-scale implied differences between candidates) still
            // dominates both.
            let wander_pen = if wander.is_some() {
                (1.0 - libm::cos(ang - th_des)) * 30.0
            } else {
                0.0
            };
            let score = steer.implied.bilinear(q) + turn_pen + baseward_pen + curl_pen
                + wander_pen + parent_pen - acc_bonus - grain_bonus;
            if score < best_score {
                best_score = score;
                best_d = cand;
            }
