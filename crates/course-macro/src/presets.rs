//! Hand-authored primitive configurations, ported from
//! `terrain-v2:golf-landform/src/presets.rs`. Kept as test fixtures (the
//! property suite runs over them) and as worked examples of realistic
//! parameter values for the planner + the macro-lab viewer's debug mode.
//! Authored on the 3 km × 3 km working box.
...
fn barranca() -> MacroConfig {
    ...
    c.tilt = Tilt { grade_x: -0.010, grade_y: -0.006, curve_m: 4.0, core_tread: 1.0 };
...
fn floodplain() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: -0.002, grade_y: 0.003, curve_m: 2.0, core_tread: 1.0 };
    c.valleys.push(Valley {
        path: Path::Meander(MeanderSpec {
...
fn canyon_straight() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: 0.0, grade_y: -0.004, curve_m: 0.0, core_tread: 1.0 };
    c.valleys.push(Valley {
...
fn ridge_spine() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: -0.003, grade_y: -0.005, curve_m: 0.0, core_tread: 1.0 };
    c.ridges.push(Ridge {
        path: Path::Points(vec![
...
fn bluff_river() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: 0.002, grade_y: -0.003, curve_m: 0.0, core_tread: 1.0 };
    c.bluffs.push(Bluff {
        path: Path::Points(vec![
...
fn bowl_spillway() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: -0.004, grade_y: -0.003, curve_m: 0.0, core_tread: 1.0 };
    c.bowls.push(Bowl {
...
fn bowl_lake() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: 0.003, grade_y: -0.002, curve_m: 3.0, core_tread: 1.0 };
    c.bowls.push(Bowl {
        boundary: BowlBoundary::Blob {
...
fn river_confluence() -> MacroConfig {
    let mut c = base(3000.0);
    c.tilt = Tilt { grade_x: -0.006, grade_y: 0.004, curve_m: 0.0, core_tread: 1.0 };
