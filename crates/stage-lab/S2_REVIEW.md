...
| 2026-08-13 | davisolmsted | S2 APPROVED on the derived engine | Reviewer pass over a seed set: "quite good; remaining oddities are S3-scope (carving expression, basins)." One tracked observation: on LOW-AMPLITUDE biomes (heathland, sandhills — seeds 34, 40) ridges can sit close together, directly parallel and straight. Diagnosis hypothesis: the carve's roughness seed is a PLANE-WAVE sum with an absolute 0.55 m floor (ROUGH_FLOOR_M, the flat-biome symmetry breaker), and on ~5 m-relief biomes the locally dominant wave's crests read as parallel straight ridges — the corduroy failure mode at the 140–520 m band. Candidate fix, not yet applied: keep the floor portion on the ROUTING surface only (like the dither) and leave only relief-proportional roughness in the terrain. Deferred with reviewer's assent ("may not have major effects") — revisit if it survives S3 texture. |
