...
fn main() {
    ...
    for a in Archetype::ALL {
        ...
        per.push((a.key().into(), gaps));
    }
    println!("{:<14} {:>5} {:>8} {:>7} {:>7} {:>6}", "biome", "n", "gap p10", "p50", "p90", "min");
    for (k, g) in per {
        if g.is_empty() { println!("{k:<14} {:>5}      --      --      --     --", 0); continue; }
        let q = |p: f64| g[((g.len() - 1) as f64 * p) as usize];
        println!("{k:<14} {:>5} {:>8.0} {:>7.0} {:>7.0} {:>6.0}", g.len(), q(0.10), q(0.50), q(0.90), g[0]);
    }
}
