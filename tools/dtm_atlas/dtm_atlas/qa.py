...
def hillshade_rgb(
    ...
    `zrange` overrides the per-image p1/p99 hypsometric stretch with an
    explicit (min, max) in meters. Pass it when two images must share one
    elevation scale — with the default self-stretch, a 2 m tile and a 90 m
    tile get the same colors and the relief difference disappears. The
    hillshade itself is unaffected: it reads true gradients either way.
    """
